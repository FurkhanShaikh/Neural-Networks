...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        if(type(shape) is not tuple):
            shape = (shape,)
        self.model.add(keras.layers.Layer(input_shape=shape,name=name,trainable=False))
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        self.model.add(keras.layers.Dense(units=num_nodes, activation=activation,
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        self.model.add(keras.layers.MaxPool2D(pool_size=pool_size, strides=strides,
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if(type(layer_numbers) is list):
            ...
            try:
                if(layer_numbers or layer_numbers==0):
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        try:
            current_weights = layer.get_weights()
            layer.set_weights([current_weights[0],biases])
        except:
            return
    ...
    def set_metric(self,metric):
        ...
        :return: none
        """
        if metric.lower() == "mse".lower():
          self.metrics = [keras.metrics.MeanSquaredError()]
        elif metric.lower() == "accuracy".lower():
    ...
    def set_optimizer(self,optimizer="SGD",learning_rate=0.01,momentum=0.0):
        ...
        if optimizer.lower() == "SGD".lower():
          self.optimizer = keras.optimizers.SGD(learning_rate=learning_rate, momentum=momentum, name='SGD')
        elif optimizer.lower() == "RMSprop".lower():
          self.optimizer = keras.optimizers.RMSprop(learning_rate=learning_rate, momentum=momentum,name='RMSprop')
        elif optimizer.lower() == "Adagrad".lower():
          self.optimizer = keras.optimizers.Adagrad(learning_rate=learning_rate, name='Adagrad')
